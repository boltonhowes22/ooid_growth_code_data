% Modified by E. Geyman (11.21.22) to account for variable major ion
% composition (e.g., [Ca], [Mg], [SO4]) and the resulting affects on K1,
% K2, Kspc, etc.

function [DATA,HEADERS,NICEHEADERS]=CO2SYS_modified(PAR1,PAR2,PAR1TYPE,PAR2TYPE,SAL,TEMPIN,TEMPOUT,PRESIN,PRESOUT,...
    Ca_input,Mg_input,SO4_input,...
    SI,PO4,NH4,H2S,pHSCALEIN,K1K2CONSTANTS,KSO4CONSTANT,KFCONSTANT,BORON)
%**************************************************************************
%
...
% Added by E. Geyman following Zeebe & Tyrrell (2019)
%global Ca_input Mg_input SO4_input
global Ca_reference Mg_reference SO4_reference
global K1_ratio K2_ratio Kspc_ratio
...
% Added by E. Geyman following Zeebe & Tyrrell (2019) -- adjustments to K1,
% K2, and Kspc (KCa) based on the varying [Ca], [Mg], and [SO4].
% Uses ratio corrections relative to modern seawater:
% Seawater reference composition from Millero et al. (2008):
Ca_reference = 10.2821; % mmol/kgSW
Mg_reference = 52.7171; % mmol/kgSW
SO4_reference = 28.2352; % mmol/kgSW
K1_ratio = 1 + 0.005*(Ca_input/Ca_reference-1) + 0.017*(Mg_input/Mg_reference-1) + 0.208*(SO4_input/SO4_reference-1);
K2_ratio = 1 + 0.157*(Ca_input/Ca_reference-1) + 0.420*(Mg_input/Mg_reference-1) + 0.176*(SO4_input/SO4_reference-1);
Kspc_ratio = 1 + 0.185*(Ca_input/Ca_reference-1) + 0.518*(Mg_input/Mg_reference-1) + 0.106*(SO4_input/SO4_reference-1);
...
Constants(TempCi,Pdbari,SO4_input);
...
[OmegaCainp(F),OmegaArinp(F)] = CaSolubility(Sal(F), TempCi(F), Pdbari(F), TCc(F), PHic(F),Ca_input);
...
Constants(TempCo,Pdbaro,SO4_input);
...
[OmegaCaout(F),OmegaArout(F)] = CaSolubility(Sal(F), TempCo(F), Pdbaro(F), TCc(F), PHoc(F),Ca_input);
...
function Constants(TempC,Pdbar,SO4_input)
...
global SO4_reference K1_ratio K2_ratio; % added by E. Geyman
...
% Edited by E. Geyman -- simple scaling of Total sulfate from SO4 -- so
% don't push to very different seawater pH

TS = SO4_input/SO4_reference * (0.14./96.062).*(Sal./1.80655); % in mol/kg-SW;
...
% Added by E. Geyman -- correct for major ion composition
K1 = K1_ratio .* K1;
K2 = K2_ratio .* K2;
...
function varargout=CaSolubility(Sal, TempC, Pdbar, TC, pH,Ca_input)
global K1 K2 TempK logTempK sqrSal Pbar RT WhichKs CAL ntps F
global PertK    % Id of perturbed K
global Perturb  % perturbation
global Kspc_ratio % added by E. Geyman
% '***********************************************************************
...
% '       13:403-417, 1968.
% '***********************************************************************
Ca=Ca_input/1000; %CAL(F); % Modified by E. Geyman to take the Ca input, division by 1000 is to convert from mmol to mol
Ar=nan(sum(F),1);
KCa=nan(sum(F),1);
KAr=nan(sum(F),1);
TempKx=TempK(F);
...
% Added by E. Geyman:
% Correct KCa for major ion composition, following Zeebe & Tyrrell (2019)
KCa = KCa .* Kspc_ratio;
KAr = KAr .* Kspc_ratio;
